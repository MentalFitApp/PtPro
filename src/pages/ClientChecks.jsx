...
const AnimatedBackground = () => <div className="absolute inset-0 -z-10 overflow-hidden bg-zinc-950"><div className="aurora-background"></div></div>;
const LoadingSpinner = () => <div className="min-h-screen flex justify-center items-center relative"><AnimatedBackground /><div className="animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-cyan-400"></div></div>;

const Notification = ({ message, type, onDismiss }) => (
    <AnimatePresence>
        {message && (
            <motion.div initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -50 }} className={`fixed top-5 right-5 z-50 flex items-center gap-4 p-4 rounded-lg border ${type === 'success' ? 'bg-emerald-900/80 text-emerald-300 border-emerald-500/30' : 'bg-red-900/80 text-red-300 border-red-500/30'} backdrop-blur-md shadow-lg`}>
...
const ClientUploadForm = ({ formState, setFormState, handleSubmit, isUploading, handleFileChange }) => {
    ...
    const PhotoUploader = ({ type, label, preview }) => (
        <div className="text-center"><label className="block text-sm font-medium text-slate-300">{label}</label><div className="mt-2 flex justify-center items-center w-full h-48 bg-zinc-900/50 rounded-lg border-2 border-dashed border-zinc-700 hover:border-cyan-500 transition-colors relative group">{preview ? <img src={preview} alt="preview" className="h-full w-full object-contain rounded-lg p-1"/> : <div className="flex flex-col items-center text-slate-400 transition-colors group-hover:text-cyan-400"><UploadCloud size={32}/><p className="mt-2 text-sm">Carica foto</p></div>}<input type="file" accept="image/*" onChange={(e) => handleFileChange(e, type)} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"/></div></div>
    );

    return (
        <div>
            <div className="flex justify-between items-center"><h3 className="font-bold text-lg mb-4 text-cyan-300">{formState.id ? 'Modifica Check' : 'Carica Check del Giorno'}</h3><button onClick={handleCancel} className="text-slate-400 hover:text-white p-1 rounded-full"><X size={20}/></button></div>
            <form onSubmit={handleSubmit} className="space-y-6">
                <div><label className="block text-sm font-medium text-slate-300 mb-2">Peso Attuale (kg)*</label><input type="number" step="0.1" value={formState.weight} onChange={(e) => setFormState(prev => ({ ...prev, weight: e.target.value }))} required className="w-full p-2.5 bg-zinc-900/70 border border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-cyan-500 text-slate-200" placeholder="Es. 75.5"/></div>
                <div><label className="block text-sm font-medium text-slate-300 mb-2">Note sul check</label><textarea value={formState.notes} onChange={(e) => setFormState(prev => ({ ...prev, notes: e.target.value }))} rows="4" className="w-full p-2.5 bg-zinc-900/70 border border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-cyan-500 text-slate-200"></textarea></div>
...
const CheckDetails = ({ check, handleEditClick }) => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center"><h3 className="font-bold text-lg text-cyan-300">Riepilogo del {check.createdAt.toDate().toLocaleDateString('it-IT')}</h3>{isEditable && <button onClick={() => handleEditClick(check)} className="flex items-center gap-2 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-slate-300 text-sm font-semibold rounded-lg"><FilePenLine size={14}/> Modifica</button>}</div>
            <div className="mt-4 space-y-4">
                <div><h4 className="font-semibold text-slate-300">Peso Registrato:</h4><p className="text-white text-xl font-bold">{check.weight} kg</p></div>
                <div><h4 className="font-semibold text-slate-300">Note:</h4><p className="text-slate-400 p-3 bg-zinc-900/50 rounded-lg whitespace-pre-wrap">{check.notes || 'Nessuna nota.'}</p></div>
                <div><h4 className="font-semibold text-slate-300">Foto:</h4><div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">{check.photoURLs && ['front', 'right', 'left', 'back'].map((type) => check.photoURLs[type] ? <a href={check.photoURLs[type]} target="_blank" rel="noopener noreferrer"><img key={type} src={check.photoURLs[type]} alt={type} className="rounded-lg w-full h-auto"/></a> : null)}</div></div>
            </div>
        </div>
    );
};
...
export default function ClientChecks() {
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            if (clientDoc.exists() && clientDoc.data().createdAt) setClientStartDate(clientDoc.data().createdAt.toDate());
            else setClientStartDate(new Date());
    ...
    const getCheckDaysForMonth = (date) => {
        ...
        while (currentCheckDate.getTime() < new Date(date.getFullYear(), date.getMonth() + 2, 1).getTime()) {
            if (currentCheckDate.getMonth() === date.getMonth() && currentCheckDate.getFullYear() === date.getFullYear()) checkDays.push(new Date(currentCheckDate.getTime()));
    ...
    const tileClassName = ({ date, view }) => {
        if (view === 'month') {
            if (checks.some(c => c.createdAt && c.createdAt.toDate().toDateString() === date.toDateString())) return 'check-submitted';
            if (getCheckDaysForMonth(date).some(d => d.toDateString() === date.toDateString())) return 'check-day';
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        } catch (error) { console.error("Errore:", error); showNotification("Si è verificato un errore."); } 
        ...
        <>
            ...
            <div className="min-h-screen text-slate-200 p-4 sm:p-8 relative">
                ...
                <header className="flex justify-between items-center mb-8">
                    ...
                    <button onClick={() => navigate('/client/dashboard')} className="flex items-center gap-2 px-3 py-2 bg-zinc-800 hover:bg-zinc-700 text-slate-300 text-sm font-semibold rounded-lg transition-colors"><ArrowLeft size={16}/><span>Dashboard</span></button>
                </header>
