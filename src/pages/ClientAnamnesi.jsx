...
const ClientAnamnesi = () => {
    ...
    // --- 4. STILI AGGIORNATI PER I COMPONENTI ---
    ...
    const headingStyle = "font-bold mb-4 text-lg text-cyan-300 border-b border-cyan-400/20 pb-2";
    ...
    const ViewField = ({ label, value }) => (
        <div>
            ...
            <p className="mt-1 p-3 bg-zinc-900 rounded-lg min-h-[44px] text-slate-200 break-words whitespace-pre-wrap">{value || 'Non specificato'}</p>
        </div>
    );
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {['front', 'right', 'left', 'back'].map(type => (
                <div key={type}>
                    ...
                    {urls?.[type] ? (<img src={urls[type]} alt={type} className="mt-2 rounded-lg w-full h-auto object-cover" />) : (<div className="mt-2 flex justify-center items-center w-full h-48 bg-zinc-900 rounded-lg text-slate-500"><span>Non caricata</span></div>)}
                </div>
            ))}
        </div>
    );
    ...
    return (
        <div className="min-h-screen text-slate-200 p-4 sm:p-8 relative">
            ...
            <header className="flex justify-between items-center mb-8">
                <h1 className="text-3xl sm:text-4xl font-bold text-slate-50">La mia Anamnesi</h1>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                {isEditing ? (
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                        {/* Sezioni del Form (struttura invariata, stili aggiornati) */}
                    ...
                    anamnesiData && <div className="space-y-8">
                        {/* Visualizzazione Dati (struttura invariata, stili aggiornati) */}
