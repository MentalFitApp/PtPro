...
export default function DashboardWidget({ id, title, icon: Icon, children, onRemove, onConfig, className = '' }) {
  return (
    <div className={`bg-slate-800/60 backdrop-blur-sm rounded-xl border border-slate-700 p-4 h-full flex flex-col ${className}`}>
        ...
        <div className="flex items-center gap-1 pointer-events-auto" style={{ pointerEvents: 'auto' }}>
          {onConfig && (
            <button
              ...
              className="text-slate-500 hover:text-blue-400 transition-colors p-1 rounded hover:bg-slate-700/50 cursor-pointer"
          ...
          {onRemove && (
            <button
              ...
              className="text-slate-500 hover:text-rose-400 transition-colors p-1 rounded hover:bg-slate-700/50 cursor-pointer"
